export default function rateLimit (instance, maxRetry = 10) {
  instance.interceptors.response.use(function (response) {
    ...
    if (error.response.status >= 500) {
      ...
      if (Date.now() - config.lastErrorAt >= 5000) {
        // we reset the attempt here if there is a gap of 5 sec
        config.attempt = 0
      }
    ...
    if (error.response.status === 429 || error.response.status >= 500) {
      ...
      wait *= Math.floor(1000 * ((Math.random() * 1.2) + 1))

      console.info(`Error occured. Waiting for ${wait}ms before retrying....`)
