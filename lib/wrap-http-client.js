...
export default function wrapHttpClient (http, {concurrency, delay, maxRetries, retryOnTooManyRequests}) {
  const backoffClient = ['get', 'post', 'put', 'delete', 'patch', 'head'].reduce(
    (http, methodName) => {
      let httpCall = http[methodName].bind(http)
      if (retryOnTooManyRequests) {
        http[methodName] = maybeBackoff(httpCall, maxRetries)
      }
      return http
    },
    cloneDeep(http)
  ...
  return rateLimit(backoffClient, ['get', 'post', 'put', 'delete', 'patch', 'head'], concurrency, delay)
}
