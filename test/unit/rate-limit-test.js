...
test('no retry when automatic handling flag is disabled', (t) => {
  ...
  return client.get('/rate-limit-me')
    ...
    .catch((error) => {
      ...
      t.equals(logHandlerStub.callCount, 1)
      t.equals(logHandlerStub.args[0][0], 'error')
      t.equals(logHandlerStub.args[0][1].message, 'Request failed with status code 500')
      t.equals(logHandlerStub.args[0][1].attempts, undefined)
      teardown()
    })
})
test('Should Fail if it hits maxRetries', (t) => {
  ...
  return client.get('/error')
    ...
    .catch((error) => {
      ...
      t.equals(logHandlerStub.callCount, 2)
      t.equals(logHandlerStub.args[0][0], 'warning', 'first log is a warning that it retries')
      t.equals(logHandlerStub.args[1][0], 'error', 'second log is an error that it failed')
      t.equals(logHandlerStub.args[1][1].message, 'Request failed with status code 501')
      t.equals(logHandlerStub.args[1][1].attempts, 2, 'logs the correct amounth of attempts')
      teardown()
    })
})
test('Rejects error straight away when X-Contentful-Request-Id header is missing', (t) => {
  ...
  return client.get('/error')
    .then((response) => {
      ...
      t.equals(logHandlerStub.args[0][0], 'error')
      t.equals(logHandlerStub.args[0][1].message, 'Request failed with status code 500')
      t.equals(logHandlerStub.args[0][1].attempts, 1)
      teardown()
    })
})
